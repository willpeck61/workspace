<?php
 //embed snippet file Plugin
 //http://www.medialab.com/sitegrinder3/support/SG3_HelpJump.html?helpsym=embed_snippet_file_xmc
 //embed a snippet of HTML from an external file.
...
{
return FETCH_URL($URL, "body", $ENVIRO);
 }
}
...
{
  echo \'--> \';
'); /*literal*/ fwrite($file, 'echo \'\';');  /*expression*/ fwrite($file, 'echo EXTRACT_TEXT(');fwrite($file, '"' .  PLUGIN_OPTION_VAL($OPTIONSDOM, "url") . '"'); fwrite($file, ', '); fwrite($file,  '$ENVIRODOM' ); fwrite($file, ');');  fwrite($file, 'echo \'\';');  /*literal*/ fwrite($file, 'echo \'\';');   fwrite($file, ' echo \' <!--\';
...
// main entry
 ...
 {
    echo '<plugin name="embed snippet file" url="http://www.medialab.com/sitegrinder3/support/SG3_HelpJump.html?helpsym=embed_snippet_file_xmc" type="expert/embed_snippet_file.sgxm.php" >
 <info>embed a snippet of HTML from an external file.</info>
