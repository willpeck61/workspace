<?php
 //php include() Plugin
 //http://www.medialab.com/sitegrinder3/support/SG3_HelpJump.html?helpsym=php_include_xmc
 //Uses the PHP include() function to dynamically import an external file. For expert users only. This importer will _not_ display correctly while viewing the page locally. Your page must have a .php extension and must be uploaded to view. 
...
');$p = 'PLUGIN_PATH("' . PLUGIN_OPTION_VAL($OPTIONSDOM, "url", $ENVIRODOM) . '", 0)';
 fwrite($file, 'include(' . $p . ');');   fwrite($file, ' echo \' <!--\';
...
// main entry
 ...
 {
    echo '<plugin name="php include()" url="http://www.medialab.com/sitegrinder3/support/SG3_HelpJump.html?helpsym=php_include_xmc" type="expert/php_include.sgxm.php" >
 <info>Uses the PHP include() function to dynamically import an external file. For expert users only. This importer will _not_ display correctly while viewing the page locally. Your page must have a .php extension and must be uploaded to view. </info>
